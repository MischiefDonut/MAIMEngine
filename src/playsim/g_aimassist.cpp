...
static void DoAimAssist(AActor * self)
{
	assert(self->player);
	...
	if(closest)
	{
		...
		/*
		Printf("Self: ( %.2f, %.2f ), Target: ( [%s] %.2f, %.2f ), Diff: ( %.2f, %.2f )\n",
			self->Angles.Yaw, self->Angles.Pitch,
			closest->GetTag(),
			target_angle.X, target_angle.Y,
			angle_diff.X, angle_diff.Y
		);
		*/
...
DEFINE_ACTION_FUNCTION_NATIVE(APlayerPawn, DoAimAssist, DoAimAssist)
{
...
CVAR (Bool, g_aimassist, false, CVAR_USERINFO | CVAR_ARCHIVE);
CVAR (Float, g_aimassiststrength, 1.0f, CVAR_USERINFO | CVAR_ARCHIVE);
CVAR (Float, g_aimassistprecision, 0.5f, CVAR_USERINFO | CVAR_ARCHIVE);
