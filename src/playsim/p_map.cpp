...
void P_FakeZMovement(AActor *mo)
{
	...
	// adjust height
	//
	const double savedz = mo->Z(); // [DISDAIN]
	mo->AddZ(mo->Vel.Z);
	if (CanFloat(mo)) // [DISDAIN]
	...
	bool missileCheck)	// [GZ] Fired missiles ignore the drop-off test
{
	...
	if (!(thing->flags & MF_NOCLIP))
	{
		...
		// [DISDAIN] Make sure the spot is a valid liquid
...
bool P_CheckMove(AActor *thing, const DVector2 &pos, FCheckPosition& tm, int flags)
{
	...
	if (!(thing->flags & MF_NOCLIP))
	{
		...
		// [DISDAIN] FIXME: Should monsters simply be allowed to get shoved out of water?
		if (thing->Vel.XY().LengthSquared() <= GetWaterCheckSpeed(thing, false) && ShouldCheckLiquid(thing))
		{
			FWaterResults res;
	...
	DAngle pitch, int flags, double sz, FLineTraceData *outdata)
{
	...
	bool ret = Trace(startpos, t1->Level->PointInSector(startpos), { pc * angle.Cos(), pc * angle.Sin(), -pitch.Sin() }, distance,
		MF_AIMASSISTTARGET, ML_BLOCKEVERYTHING, t1, trace, TRACE_NoSky, CheckAimAssistTrace, nullptr);
