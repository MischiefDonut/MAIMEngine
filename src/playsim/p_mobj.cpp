...
void AActor::Serialize(FSerializer &arc)
{
	...
	arc
		...
		A("flags8", flags8)
		A("flags9", flags9)
		A("disdainflags", DisdainFlags) // [DISDAIN]
		...
		A("watertop", watertop)			// [DISDAIN]
		A("waterbottom", waterbottom)	// [DISDAIN]
...
void AActor::SplashCheck()
{
	...
	{
		P_HitWater(this, Sector, PosAtZ(watertop), true); // [DISDAIN]
	}
...
void AActor::Revive()
{
	...
	flags7 = info->flags7;
	flags8 = info->flags8;
	DisdainFlags = info->DisdainFlags; // [DISDAIN]
...
void PrintMiscActorInfo(AActor *query)
{
	if (query)
	{
		...
		// [DISDAIN]
		Printf("\n   DisdainFlags: %x", query->DisdainFlags.GetValue());
		for (flagi = 0; flagi <= 31; flagi++)
			if (query->DisdainFlags & DisdainActorFlags::FromInt(1 << flagi)) Printf(" %s", FLAG_NAME(1 << flagi, DisdainFlags));
